...
When Faugere says 'reductors' he's talking about choosing divisors during
polynomial division. By Left and Right of the critical pair, he is talking
about each half of the S-polynomial.
...
# TODO - TRY USING SparseMatrixCSC via SuitSparse.jl and SparseArrays.jl
...
function F4_naive(G::Vector{T})::Vector{T} where {T <: AbstractPolynomial}
	...
	# sort!(B; by = pair -> degree(leading_monomial((lcm(G[pair[1]], G[pair[2]])))))
	n = length(B)
	...
	while !isempty(B) 
		...
		m = max(4, length(B) ÷ 3) 
		# d = degree(leading_monomial((lcm(G[B[1][1]], G[B[1][2]]))))
		# m = findfirst(pair -> degree(leading_monomial((lcm(G[pair[1]], G[pair[2]])) != d)), B)
		# m = max(4, m) 
		# m = min(m, length(B) ÷ 3)
		...
		# Markowitz pivoting may improve this asymptotically?
		...
		# TODO - Try getting intermediate reductions - G = reduce_gb(G)
		# TODO - Do this in a way that we don't keep repeating the same pairs ever
		if length(B) > 2*n
			G = reduce_gb(G)		
			B = [(i, j) for i in 1:length(G) for j in i+1:length(G)]
			# sort!(B; by = pair -> degree(leading_monomial((lcm(G[pair[1]], G[pair[2]])))))
